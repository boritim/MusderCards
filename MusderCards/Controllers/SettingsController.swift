...
class SettingsController: UITableViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 2:
            cell.textField.placeholder = "Профффессия"
            cell.textField.text = user?.profession
            cell.textField.addTarget(self, action: #selector(handleProfessionChange), for: .editingChanged)
        case 3:
            cell.textField.placeholder = "Возраст"
            if let age = user?.age {
                cell.textField.text = String(age)
            }
            cell.textField.addTarget(self, action: #selector(handleAgeChange), for: .editingChanged)
        default:
            cell.textField.placeholder = "Биография"
        }
    ...
    @objc fileprivate func handleSave() {
        ...
        Firestore.firestore().collection("users").document(uid).setData(docData) { (err) in
            hud.dismiss(animated: true)
            if let err = err {
                print("Ошибка с сохранением настроек пользователя:", err)
                return
            }
            print("Сохранение настроек профиля удачно")
